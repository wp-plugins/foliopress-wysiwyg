...
$kfm->defaultSetting('default_directory_permission',777);   //changed from 755 zUhrikova
...
$kfm->defaultSetting('use_multiple_file_upload',$bUseFlashUloader);
...
$kfm->defaultSetting('max_image_upload_width', $iMaxWidth);
$kfm->defaultSetting('max_image_upload_height', $iMaxHeight);
// plugins
...
if (is_dir($workpath)) {
    ...
    if (is_writable(dirname($workpath_tmp)))mkdir($workpath_tmp, 0777);
    else{
...
case 'sqlitepdo': // {
    ...
    $dsn   = array('type'=>'sqlitepdo', 'database'=>WORKPATH.DBNAME, 'mode'=>'0666');
    $kfmdb = new DB($dsn);
...
function kfm_getFilesUrlAndSizes( $id, $x=0, $y=0 ){
		...
		$aReturn[] = getimagesize($_SERVER['DOCUMENT_ROOT'] .$aReturn[0]);//$aReturn[0]
