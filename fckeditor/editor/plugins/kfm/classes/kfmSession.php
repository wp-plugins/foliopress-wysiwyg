...
class kfmSession extends kfmObject{
	...
	function set($name='',$value='',$save_in_db=true){
		...
		if($save_in_db){
			...
			$kfm->db->query("INSERT INTO ".KFM_DB_PREFIX."session_vars (session_id,varname,varvalue) VALUES (".$this->id.",'".sql_escape($name)."','".sql_escape(kfm_json_encode($value))."')");
		}
	}
	...
	function get($name){
		...
		if($res && count($res)){
			$ret=kfm_json_decode('['.stripslashes($res['varvalue']).']',true);
