...
<h4>Header 4</h4>';
  ...
  function AdminNotices() {
    ...
    if( $this->aOptions['kfmlang'] != 'auto' ) {
      ...
      $html .= '<p class="hide-if-no-js"><a title="Set Featured image with SEO Images" href="#" id="seo-images-featured-image" '.$onclick.'>Set featured image</a></p>';
  	  return $html;
	...
	function checkUserAgent() {
	  if( stripos( $_SERVER['HTTP_USER_AGENT'], 'MSIE 9.0' ) !== FALSE ) return true;
	  else return false;
	}
  ...
  function content_edit_pre($content) {
  		...
  		if( $meta['post_modified'] == $post->post_modified ) {
  		  return $content;
  		} 		
  ...
  #quicktags { display: none; }
	...
	function featured_image() {
	  ...
	  $file = $wp_upload_dir['basedir'].preg_replace( '~^'.$wp_upload_dir['baseurl'].'~', '', $_POST['imageURL'] );
	...
	function fv_remove_mediabuttons($content) {
      ...
      if( isset( $aOptions['HideMediaButtons'] ) && $aOptions['HideMediaButtons'] == true)
				return $content.'<style>#media-buttons { visibility: hidden; }</style>';
			else
				return $content;
   }
...
<?php 
  ...
  function the_editor( $content ) {
    if( $this->checkUserAgent() ) {
      $content = '<p style="border-radius: 3px 3px 3px 3px; border-style: solid; border-width: 1px; padding: 0 0.6em; background-color: #FFEBE8; border-color: #CC0000; ">Internet Explorer 9 is currently having issues with Foliopress WYSIWYG. Please use Safari, Firefox, IE 7, IE 8 or Chromium!</p>'.$content;
    }
    return $content;
  }
